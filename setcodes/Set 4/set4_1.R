...
#4:
z_rev <- -z + max(z) + 0.1  #makes everything negative; max = 1.3 and min = 0.1
...
ggplot(data = df2, 
  ...
  geom_vline(xintercept = mean(z_rev), linetype = 'dashed', color = 'blue') +
  geom_vline(xintercept = prod(z_rev)^(1/length(z_rev)), color = 'purple') +
  geom_vline(xintercept = median(z_rev), color = 'green') 
  #coord_cartesian(xlim=c(0.4, 0.6))  #zoom without losing data points

  #adding negative changes skew from Right to Left
